...
dataset1 <- Generate_Dataset_BL(5,10,N) # 5 real param, 10 noise param
x1 <- dataset1[[1]]
y1 <- dataset1[[2]]$V1
betas1 <- dataset1[[3]]
...
posterior1 <- extract(fit1, inc_warmup = TRUE, permuted = FALSE)

color_scheme_set("red")
betas <- paste0('beta[', 1:15,']')

post_plot_fit1 <- mcmc_areas(posterior1,
           pars = betas,
           prob = 0.8) + ggtitle("Posterior distributions of Fit1",
                      "with medians and 80% intervals")
post_plot_fit1
ggsave("Plots/post_plot_fit1.jpg")
...
color_scheme_set("red")
betas <- paste0('beta[', c(1:5, 10, 17, 42, 76, 104),']')

post_plot_fit2 <- mcmc_areas(posterior2,
           pars = betas,
           prob = 0.8) + ggtitle("Posterior distributions of Fit2",
                      "with medians and 80% intervals")
post_plot_fit2
ggsave("Plots/post_plot_fit2.jpg")
print(fit2)
```
...
dataset3 <- Generate_Dataset_BL(5, 45, N)
x3 <- dataset3[[1]]
y3 <- dataset3[[2]]$V1
betas3 <- dataset3[[3]]

# Fit Stan Model
fit3 <- stan(file = "Lasso1.stan",
			 data = list(N = N,
			 			K = ncol(x3),
			 			y = y3,
			 			X = x3),
...
posterior3 <- rstan::extract(fit3, inc_warmup = FALSE, permuted = FALSE)

color_scheme_set("red")
betas <- paste0('beta[', c(1:5,10,25,37, 42, 50),']')

post_plot_fit3 <- mcmc_areas(posterior3,
           pars = betas,
           prob = 0.8) + ggtitle("Posterior distributions of Fit3",
                      "with medians and 80% intervals")
post_plot_fit3
ggsave("Plots/post_plot_fit3.jpg")
...
res <- read.csv("Results Summary.csv")
